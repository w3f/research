...
We next describe the protocol by which the parachain validators $\vals_\para$ assigned to $\para$ announce parachain candidates to the full validator set $V$ for including in a relay chain block.
...
TODO: Address liveness for candidate receipt $\receipt_{B,S}$

We gossip an abridged candidate receipts, along with their attestations signature set $S$, among the parachain validators $\vals_\para$, so they obtain the full attested candidate receipts $\receipt_{B,S}$.  Any parachain validators $V'$ in $\vals_\para$ who receives a live $\receipt_{B,S}$ downloads $\blobB$ from $C$ or another validator who attests in $S$.  After receiving $\blobB$, then they gossip with their own attestation $S := S \cup \{ (V',\sigma') \}$

In gossiping $\receipt_{B,S}$, we further aggregate these attestations signature set $S$.\footnote{We envision $\vals_\para$ being small enough that BLS signatures do not improve verification time over Schnorr signatures, although BLS might reduce the candidate receipt's signature from 640 bytes down to 50 bytes.}  
If $S$ accumulates at least $\primarychecks$ attestations from distinct validators in $\vals_\rho$, then  we publish $\receipt_{B,S}$ for relay chain block producers using relay chain gossip (mempool).
...
\label{subsec:availability}  % Pre-GRANDPA
