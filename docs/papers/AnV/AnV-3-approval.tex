...
As noted above, any validator $V$ accumulates approval checks $S$ in an attested candidate receipt $\receipt_{B,S}$.  All our assignment schemes come with applicability criteria, like $R$ having equivocations, as well as fulfilment criteria, like $|S_z| \ge \ell + \theta \ell'$.  We judge $R$ approved when all applicabile assignment schemes appear fulfilled.  An approved relay chain block $R$ becomes eligible for consideration by our finality gadget GRANDPA.
