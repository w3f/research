...
As mentioned above in \S\ref{sec:availability}, any validator $V$ accumulates availability announcements for each relay chain block $R$.  After $V$ observe $2f+1$ such availability claims, then $V$ considers $R$ {\em available} and enters the approval phase for $R$.  The approval phase for $R$ consist of
\begin{itermize}
\item VRF criteria that determine the parachain candidates in $R$ on which $V$ runs secondary checks,
\item announcements by secondary check criteria winners,
...
We recall that a relay chain block $R$ has equivocations exist if there exists another relay chain block $R'$ distinct from $R$ using the same VRF output $\omega_{R'} = \omega_R$ for the relay chain randomness.  We distrust relay chain blocks with equivocations of course, and must mark them as having equivocations, but we sadly cannot invalidate them because doing so creates attacks on finality.  TODO: Explain?

\paragraph{Criteria:}

...

We shall define four secondary check criteria on a parachain candidate receipt $\reciept_{B,\cdot}$ on two axes: initial vs completing, and whether the relay chain block $R$ has equivocations.  We define one fifth secondary check criteria to ...

We track the number $\theta_{B,R}$ of secondary checks for $B$ associated to the relay chain block $R$.  If $R$ has equivocations, then we also track the number $\theta_B$ of secondary checks for $B$ not associatred to any relay chain block.  All secondary checks accumulate in the set $S$ of signatures on the candidate receipt $\reciept_{B,S}$.  If $B$ is a block on parachain $\rho$, then any parachain validator for $\rho$ in $\vals_\rho$ gets counted in both $\theta_B$ and $\theta_{B,R}$ for some $R$.  All other validators get counted only if they also published their VRF proof for winning a secondary check criteria for $B$.  We have no single criteria for being counted in both $\theta_{B,R}$ for some $R$ and $\theta_B$, meaning adding a validator to $\theta_{B,R}$ or $\theta_B$ never results in also adding the same validator to $\theta_B$ or $\theta_{B,R}$, respectively, or to $\theta_{B,R'}$ for some other $R'$, although conceivably some might get counted by winning multiple criteria.
...
In all cases, $V$ takes these steps once it considers some secondary checks VRF criteria satisfied:

First, $V$ construsts its secondary checks claim $\zeta$, which includes any equivocations required to verify $\zeta$.  Among our claim types, two consider the current secondary checks count and relative time as perceived by $V$, but verifiers compute this data locally.  $V$ announces its claim $\zeta$ via gossip.

Second, we let $B$ denote the parachain candidate determined by $\zeta$ and $\reciept_{B,\cdot}$ its candidate receipt.  $V$ reconstructs the full {\em candidate proof-of-validity blob} $\blobB$ by obtaining enough pieces $\pieces_B$ from nodes known to posses them.  

$V$ knows which which nodes have their individual pieces by the availability announcements.  Also $V$ knows that all pieces are known by the primary checkers aka parachain validators who approved $\blobB$, as well as secondary checkers who already approved $\blobB$.  Ideally $V$ might retrieve the pieces in $\pieces_B$ only using its existing connections in our topology specified above, except these intentionally do not include 1/3rd of validators.  Also, $V$ need not connect to any node with all of $\pieces_B$. 

We further caution that verifying the VRF in $\zeta$ incurs computational costs similar to initiating a connections.  We therefore suggest that $V$ first contact some node that possesses all of $\blobB$, and only then begin a full reconstruction process.  
