...
A parachain $\para$ refers to its block producers as {\em collators}, which could come from $\vals$ or elsewhere.  We let $\vals_\para \subset \vals$ denote the sublist of validators assigned to parachain $\para$ \handan{}{and call them {\em parachain validators}}.  We also need a bound $\npvals$ such that $|\vals_\para| \ge \npvals$.
...
A\handan{ {\em rigid/cryptographic $k$-of-$n$}}{what is rigid/cryptographic?} erasure code consists of two algorithms:
\begin{itemize}
