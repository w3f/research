...
We say an adversary wins the {\em naively sharded $\npvals$-validity game} if they first learn the role assignments for all nodes, only then proposes an invalid shard block $B$, and convince the relay chain finalize $B$, all while fewer than $\npvals$ of the $f'$ incorrectly behaving nodes provide proofs of their incorrect behavior to the correctly behaving nodes. 

There are protocols like ByzCoin \cite{ByzCoin} and .. that assign $\npvals$ validators to each shard in order to argue that any adversary wins some random instance of a naively sharded $\npvals$-validity game with odds $\epsilon'_{\npvals}$ of order roughly ${f' \choose \npvals} / {\nvals \choose \npvals}$.  In these, security depends upon $\epsilon'_{\npvals}$ being ``negligible'' so that adversaries cannot simply wait until they win the shard assignment lottery before launching an attack.  For this, one requires that $\npvals$ be a significant fraction of $\nvals$, and that $\nvals$ itself be large enough. 
%TODO: End of not really true part.
%  Fix by cite ByzCoin, etc. correctly.  Eleftherios? 

We mildly distrust requirements for a large network size $n$ for two reasons: Almost all networks start small at least in the sense of truly independent nodes, and the finality gadget and its gossip assumption limit $n$.  We worry much more about requirements for a large number $\npvals$ of validators assigned to each shard because this limits the number $n/\npvals$ of shards, and hence the value of our scaling effort.
...
We say an adversary wins the {\em parachain $\primarychecks$-validity game} if they first propose an invalid parachain block $B$, and only then learn the nodes assigned to check the block, all while fewer than $\primarychecks$ of the $f'$ incorrectly behaving nodes provide proofs of their incorrect behavior to the correctly behaving nodes. 

Instantiating this game requires several distinct but related paramaters:  
