...
$$ \mathcal{S} &:= \mathtt{trim}_\vrfaslots\left( \mathtt{sort} \Listst{
...
Instead we suggest to sort $\omega'_{V,e,i}$ so the smallest $\omega'_{V,e,i}$ values most likely to survive all sub-phases play the most important roles, ignoring $r'_{e+l}$, and then trim the resulting sorted list to only the smallest $\vrfaslots$ final winners. 
$$ 
\mathcal{S} &:= \mathtt{trim}_\vrfaslots\left( 
\mathtt{sort} \Listst{
...
$$ 
\mathcal{S} := 
  \mathcal{S}_0[ 1,2,\ldots, 2 \floor{{\vrfaslots-1 \over 2}} + 1 ]
\|
  \mathtt{reverse}( \mathcal{S}_0[ 0,2,\ldots, 2 \floor{\vrfaslots/2}] )
$$
An outside-in sort still permits an adversary to trade beginning runs for ending runs, but this sounds mostly harmless.

Any of the above tree approaches should prevent an adversary from sacrificing some of their own blocks to position a long streak of their own blocks towards the end of the epoch.  
...
We could naively adapt Praos \cite{Praos} to a VDF by defining its $r_e$ to be the VDF output when given $\Omega^{\labelVRFr}_{e-\epochsdelayforVDF}$ where $\epochsdelayforVDF$ epochs is longer than $\Twait$.  Yet, we cannot similarly take $r_e = r'_e = r''_e$ to be the VDF output because doing so exposes our sorting randomness $r'_{e+l}$ too early: 

An adversary could learn $r'_{e+l}$ only $\Tsec$ after its VDF started.  We recall that $r'_{e+l}$ gets used in finalising the sortition phase that start in epoch $e$.  As $\Tsec$ would always be much less than an epoch, our VDF for $r'_{e+l}$ must start after the end of the first three sub-phases of sortition, and must be available to end sortition.  It follows that $\epochsdelayforVDF$ aka $\Twait$ must be less than $l$, which limits our VDF security dramatically.  At first blush, we now ask that one VDF end for each epoch, which then requires running almost $l$ VDFs in parallel.

We could however define only $r_e$ to be the VDF output and take simply $r'_e = \Omega^{\labelVRFr}_{e-1}$, so $r_e \ne r'_e$, if we want to keep $l$ relatively short.  We must analyse if $r'_e$ really improves our security much in this case.

TODO: More complete half VDF design
