...
We should send $(i,\omega_{V,e,i},\pi_{V,e,i})$ to $U$ as directly as possible, but always over an encrypted and authenticated channel.  At worst, $V$ could gossip a cyphertext $C$ that encrypts $(i,\omega_{V,e,i},\pi_{V,e,i})$ to a public key $U.\pk$ of $U$, and sign $(j,C)$ with $V.\pk$ where $j < \vrfawinnersbound$ is unique among such ciphertexts originating from $V$.  In other words, $\vrfawinnersbound$ acts as another de facto bound on the number of $(i,\omega_{V,e,i},\pi_{V,e,i})$ published.

In the second sub-phase, each repeater $U$ publishes on-chain at most $\vrfarepeatbound$ such anonymised VRF outputs $(\omega_{V,e,i},\pi_{V,e,i})$ for various block producers $W$.  These transactions record $(i,\omega_{V,e,i},\pi_{V,e,i})$ in the chain's associated state, so they should check the proof $\pi_{V,e,i}$, forbid duplicates, and enforce the above conditions.
...
Around the end of epoch $e+l-1$, we sort the $H(\omega'_{V,e,i} || "IF")$ so that values most likely to survive previous epoch appear earliest.  We then trim this list to only $\vrfaslots$ final winners, and reverse the order, so the smallest $H(\omega'_{V,e,i} || "IF")$ values appear towards the end.  We declare the result to be the slot allocations for epoch $e+l$.
\begin{align*}
\mathcal{S}_0 &:= \mathtt{sort} \Listst{ 
  (\omega'_{V,e,i}  || "IF"),W,i) 
}{ 
  \omega'_{V,e,i} \mathrm{ correctly made it on-chain}
} \\
\mathcal{S} &:= \mathtt{reverse}( \mathtt{trim}_\vrfaslots(\mathcal{S}_0 ) )
\end{align*}
We do this trim and reverse trick so that the $\omega'_{V,e,i}$ in the most powerful positions towards the end of epoch $e+l$ were always the most likely to be included.  If one does not reverse then an adversary can sacrifice some of their own blocks to position a long streak of their own blocks towards the end of the epoch.  In principle, one could replace $"IF"$ here with $r'_{e+l}$ provided $l>1$, so that only $r'_{e+l}$ controls the ordering and positioning of runs, but the reverse and trim trick appears sufficient. 
