...
\item$\VRF.\Sign_{\skvrf}$ takes a secret key $\skvrf$ series of inputs $(\alpha_1,\ldots,\alpha_n)$ with $n>0$  \comment{and a message $m$}{what is this $ m $? why don't we integrate it to $ \alpha $ values?}, and produces a series of outputs $(\omega_1,\ldots,\omega_n)$ such that $\omega_i$ is determined by $\alpha_i$, and a proof $\pi$, not necessarily deterministic.
