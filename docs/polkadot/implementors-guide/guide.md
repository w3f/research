...
Optimistic scheduling for parathreads should respect the priority of claims. Regardless of which previously-occupied parathread execution core is freed, the same parathread should be assigned to occupy that core afterwards. The same goes for parathread-retry cores. This implies that the information made available to the Node-side should contain information about:
  * Which cores are currently assigned to which parathreads
  * The queues of core assignments that correspond to disjoint sets of currently occupied cores (i.e. the parathread queue and the parathread retry queue)
  * The necessary conditions for each occupied core to be freed. This information comes from the Inclusion module, which handles availability.

With this information, the Node-side can be aware of which parathreads have a good chance of being includable within the relay-chain block and can focus any additional resources on backing candidates from those parathreads. If the necessary conditions are reached for core reassignment, those backed candidates can be included within the same block as the core being freed.
