...
This is the core idea of our method. In the remainder of the section we establish how to find the candidate with the largest prescore efficiently.
...
(ii) $d^*=score_w(v')$ is the unique root of function $g(d):= score_w(v',d) - d$.
...
To prove point $(iii)$, let $d^*:=score_w(v')=score_w(v',d^*)$, and consider first a value $d<d^*$ so that $0\leq d<d^*$. If $score_w(v',d)=d^*$ then there is nothing to prove. Otherwise, we have that 
...
where we used the fact that \(d^*\cdot denom(d^*)=num(d^*)\) because \(g(d^*)=num(d^*)/denom(d^*) - d^* = 0\). From the previous inequality, we conclude that $d^* \geq num(d)/denom(d) = score(v',d)$, as desired.
...
**Corollary 3.** Fix a maximally affordable solution $(S,w)$. Then, the function \(\max_{v'\in V\setminus S} score_w(v',d)\) is constant in each of the above-defined intervals $[0,d_1), \ [d_1, d_2), \cdots, [d_k, \infty)$, and it reaches its maximum over the point \(d^*=\max_{v'\in V\setminus S} score_w(v')\), where its value of $d^*$.
