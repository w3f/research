...
$$score_w(v',d_i)= \frac{\sum_{n\in N: \ nv'\in E} (b_n - \sum_{v \in S: \ supp_w(v) \leq d_i} w_{nv})}{1+\sum_{n\in N: \ nv'\in E} \sum_{v \in S: \ supp_w(v) > d_i} w_{nv} /supp_w(v) }.$$

5. Let $(d^*, v^*)=(\max, \arg\max)_{v'\in V\setminus S} score_w(v',d_i)$.
...
**Lemma 4:** CalculateMaxScore\((S,w)\) returns \(\max_{v'\in V\setminus S} score_w(v')\) and a $v'$ that attains that socre, in $O(\log |S|)=O(\log m)$ iterations, where each iteration takes time $O(|E|)$.
