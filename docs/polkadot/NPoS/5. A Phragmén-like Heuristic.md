...
**Proof:** The inequality \(1+x\leq e^x\) holds for any real $x$. Replacing $x$ by $\varepsilon/(5m)$ and raising both sides by the power $m$, we obtain
...
c) For each $v\in S$ such that $w_{nv}>0$ for some $n\in N_a$, we have that \(supp_{w}(v)\geq a\cdot d^*\).
...
* $\nexists n\in N_l, v\in S_u$ such that $w_{n,v}>0$: Assuming otherwise that there is such a pair, the fact that $n$ is in $N_l$ implies that it has a neighbor $v'\in S_l$. But then we can apply Condition 1.(iii) to obtain $supp_w(v)/supp_w(v')\leq 1+\epsilon/(5m')=d_u/d_l$, which contradicts the definitions of $S_l$ and $S_u$. 
