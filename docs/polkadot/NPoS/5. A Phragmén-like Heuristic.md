...
Checking whether a committee $(S,w)$ satisfies standard PJR is known to be NP-hard. However, for any $d\geq 0$ we can efficiently check whether $\max_{v'\in V\setminus S} score_w(v')<d$, and if this in turn implies PJR($d$).
...
1. Compute $supp_w(S)$. Let \(d=\min \{(1+\epsilon)\cdot supp_w(S), \sum_n b_n/|S|\}\)/
