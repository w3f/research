...
\begin{abstract}
In recent years a vast number of blockchain projects have been introduced and employed with various features that are not necessarily designed to work  with each other.  This makes is difficult for user to utilise a large number of application on different block chain projects. Moreover, with the increase the number of projects the security each one is providing individually becomes smaller. In this paper we describe the design components of the heterogenous multi-chain protocol Polkadot that addresses these shortcomings.
We describe the properties that Polkadot aims to achieve and explain all its components.
