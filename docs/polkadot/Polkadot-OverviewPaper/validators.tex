...
Polkadot will use Nominated Proof-of-Stake (NPoS), \handan{our}{} own version of proof-of-stake (PoS).
Consensus protocols with deterministic finality, such as \handan{ours}{maybe it is better to use `Polkadot' than saying `ours'}, require a set of registered validators of bounded size.
...
(see more details on \handan{the Economics section}{add reference to this section}).
...
proposed a \handan{method}{refs} for electing members to his country’s parliament.
...
the protocol will select a set $\Val\subseteq \Can$ of $\nval$ validators with \handan{the following property}{it may be better to say in words why the following mathematical definition represents PJR property.}: \\
...
with the PJR property, but also \handan{a distribution of each nominator's stake among the elected validators that she backs}{verb of the sentence is missing},
