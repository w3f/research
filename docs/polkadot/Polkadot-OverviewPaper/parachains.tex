...
XCMP is the protocol that parachains use to send messages that guarantees messages arrive quickly, messages from one parachain to another arrive in order,
and messages that arrive were indeed sent in the final history. As a result of these properties, we will need to require that a parachain accepts all incoming messages.
