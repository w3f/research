...
There is an ordering of all parachain blocks, which is we first sort by relay chain block number and then break ties for that by parachain number. The watermark must advance parachain block in this order. The watermark must advance by at least one parachain in every parachain block. To produce a parachain block on parachain $D$ which builds on a particular relay chain block $R$, a collator would need to look at which parachain headers were built between the relay chain block that the last parachain block of this chain built on,
