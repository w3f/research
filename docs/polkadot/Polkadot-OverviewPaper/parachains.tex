...
\leon{If that is in the best chain according to BABE (See Section \ref{sec:babe}), so is the parachain block and when that is finalised, so is the parachain block.}{This sentence is a bit confusing with too many that's}
...
As a result of these properties, we require that a parachain accepts all incoming messages.
...
\ximin{}{TODO: the following parargaphs talk about integrity; we should briefly mention availability}
