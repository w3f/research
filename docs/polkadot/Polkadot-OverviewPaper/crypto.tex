\subsection{Cryptographic Keys}\label{sec:crypto}
...
% TODO: I've no idea what this footnote wants since it goes into this below.
...
A VRF is the public-key analog of a pseudo-random function (PRF), aka cryptographic hash function with a distinguished key, such as many MACs.  We award block production slots when the block producer scores a low enough VRF output \handan{$\mathtt{VRF}(r_e || \mathtt{slot_number} )$}{it does not have the secret key as an input}, so anyone with the VRF public keys can verify that blocks were produced in the correct slot, but only the block producers know their slots in advance via their VRF secret key.
