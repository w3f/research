...
A session public key should consist of three or four types of public keys: 
 ...
 - Ristretto Schnorr public key (32-byte public keys, 64-byte signatures, 96-byte VRFs)
   ...
   Signers can efficiently batch numerous VRF outputs into a single proof using these keys, similar to CloudFlare's Privacy Pass.  If block production employs these VRFs, signers could periodically publish a "sync digest" to consolidate thousands of past block production VRFs into a single verification, significantly improving syncing speed.  Additionally, there is a pathway to batch-verify these VRFs across multiple signers, which would require enlarging the VRF output and proof size from 96 to 128 bytes.

 - Small curve of BLS12-381 (48-byte public keys, 96-byte signatures)

   This key can verify aggregated signatures more efficiently if the signer set for a particular message is large but irregularly composed, as in GRANDPA. Individual signature generation is slower compared to the the reverse orientation, and the use of non-constant-time extension field arithmetic further increases latency and introduces potential security risks.  Aggregating signatures on the same message also introduces malleability risks.  This scheme may be applicable in certain fishermen protocols.
 ...
 - Big curve of BLS12-381 (96-byte public keys, 48-byte signatures) (optional)
   ...
   Aggregate verification is likely to run faster when signer sets are frequently reused, making this scheme suitable even for contexts where small-curve keys might initially seem optimal. Signature aggregation in GRANDPA is likely to be "wild," which makes the small-curve key a better fit in that setting.
