...
<!--
A blockchain protocol requires a leader election mechanism to select nodes that will produce blocks. Proof of Work (PoW) is a popular method for such election, where nodes with faster block production with a block hash values below a threshold become leaders. Some Proof-of-Stake (PoS) protocols also adopt a similar approach based on stake instead of computational power. These approaches, although communication-free and some providing leader secrecy, result in multiple leaders or even no leader due to threshold conditions not being met. This leads to wasted efforts and time [8].

In this post, we present Sassafras, our PoS-based blockchain protocol that deploys a novel and efficient election mechanism. Sassafras [1] is proposed as a replacement for the BABE+Aura consensus mechanism in Polkadot, offering several advantages over existing PoS protocols. -->
...
<!--Now, let's assess the performance of Sassafras in terms of computation, communication, and security levels. -->
